...
public final class VipsOutput {
	...
	private final int COLOR_STEP = 15;
	...
	private void drawVisualBlocks(Graphics2D g, VisualStructure visualStructure, boolean strictRendering)
	{
	    int doc = visualStructure.getDoC();


	    if (!strictRendering || _pDoC == doc)
	    {
    	    if (this.currentColor >= this.COLOR_STEP) this.currentColor -= this.COLOR_STEP;
    		g.setColor(new Color(this.currentColor, this.currentColor, this.currentColor));
    		g.fillRect(visualStructure.getX(), visualStructure.getY(), visualStructure.getWidth(), visualStructure.getHeight());
    		g.setColor(Color.BLACK);
    		g.drawString(String.valueOf(doc), visualStructure.getX()+2, visualStructure.getY()+12);
	    }

		if (_pDoC >= doc)
		{
			for (VisualStructure child : visualStructure.getChildrenVisualStructures())
				drawVisualBlocks(g, child, strictRendering);
		}

		if (!strictRendering || _pDoC == doc)
		{
		    if (this.currentColor < 255) this.currentColor += this.COLOR_STEP;
		}
	}

    public void drawImage(VisualStructure visualStructure, Viewport pageViewport, boolean strictRendering)
    {
        BufferedImage boxImg;
        Graphics2D g;
        int width = pageViewport.getWidth();
        int height = pageViewport.getHeight();

        boxImg = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        g = boxImg.createGraphics();
        g.setColor(Color.white);
        g.fillRect(0, 0, width, height);

        drawVisualBlocks(g, visualStructure, strictRendering);

        File outputfile = new File(_filename+".png");
        try {
